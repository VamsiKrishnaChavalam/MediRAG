"""
MediRAG - AI Medical Assistant
A Streamlit application for medical document analysis with RAG (Retrieval-Augmented Generation)
"""
...
def initialize_services():
    """Initialize all external services (LLM, Pinecone, embeddings, etc.)"""
    try:
...
def chunk_page_text(page_dict, chunk_size=800, chunk_overlap=200):
    """
    Create chunks from a page using LangChain's RecursiveCharacterTextSplitter 
    while preserving metadata for citation.
...
def format_sources_from_docs(docs):
    """Create deterministic sources output from retrieved docs metadata"""
    seen = set()
    items = []
    for d in docs:
...
def build_rag_prompt(query, message_history, docs):
    """Build comprehensive RAG prompt with strict PDF-only instructions"""
    history_text = ""
    if message_history:
...
def judge_score(client, model, question, answer, contexts, dimension: str):
    """LLM-as-Judge scoring for different dimensions"""
...
def evaluate_realtime(client, model, question, answer, contexts):
    """Run real-time evaluation across all dimensions"""
    start = time.time()
...
# =============================================================================
# SIDEBAR - FILE UPLOAD & SYSTEM STATUS
# =============================================================================
        ...
        if uploaded_files:  
            ...
            for uf in uploaded_files:
                with st.spinner(f"Processing {uf.name}..."):
                    ...
                    if not pages:
                        st.markdown(
                            f'<div class="status-message error-message">Choose OCR to upload a scanned document"{uf.name}".</div>',
                            unsafe_allow_html=True
                        )
                        continue
